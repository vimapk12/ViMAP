...
public final class RectangleShapeChecker implements ShapeChecker {

    /**
     * Expects a rectangle of the form lower-left-x, lower-left-y,
     * width, height.
     * 
    ...
    public String checkShape(final List<Double> values) {
        ...
        if (values.size() != expectedNumValues) {
            return "You did not draw a rectangle.";
        }
        ...
        if (rightX <= leftX) {
            return "The rectangle must have width.";
        }        
        if (bottomY <= topY) {
            return "The rectangle must have height.";
        }
