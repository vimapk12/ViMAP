...
to setup 
  ...
  ;prepare-all-blocks
  let preparations ""
  foreach pc:list-all "prepare-" [
   set preparations (word preparations " " ? " ") 
  ]
  run preparations
...
to table-of-contents-for-all-blocks  ;;just for documentation
...
to-report get-measures-for-starting-with [an-agent-kind start-index]
  ...
  [
    foreach (sublist relevant-list start-index (length relevant-list) ) ;;others have len rel list - 1
    [
      ask ? 
      [ 
        let datarep (list who red (word "\"" tagentkind "\"") tcycles tpopulation antEnergy spiderpopulation frogpopulation theading tdistfromlast tspeed taccel tpenwidth tpencolor) 
