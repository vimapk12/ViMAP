...
;  set blocks-list lput max-one-of blocks [who] blocks-list
  ...
  create-blocks 1  [
    ...
    [
      set arg-type "enum"
      set enum-list ["turtle" "ant" "butterfly" "car" "cow" "fish" "person" "squirrel"]
    ]
    set arg-list lput max-one-of args [who] arg-list
    set is-observer false
    set is-basic false
...
to java-place-measure-point
  ...
  [
   ...
   set tcycles [flag-counter] of myself ; previously was count measurepoints - 1
...
; This code is called by DomainModelImporter.java
; You pass it a list name blocks-list 0 "block-name"
; It reports the value of block-name on index 0 in block-list. 

; runresult takes a task or a String.  In this case we use a string.  Notice \" in Java code.
...
to-report get-measures
  ...
  [
    ...
    [ 
      ...
      let datarep (list who tcolor (word "\"" tagentkind "\"") tcycles theading todometer tdistfromlast tspeed taccel tpenwidth tpencolor tagentsize) 
...
to-report get-measures-for-filtered [an-agent-kind a-measurepoint-creator]
  ...
  [
    ...
    [ 
      ...
      ;; (length result + 1)
      let datarep (list who tcolor (word "\"" tagentkind "\"") (length result + 1) theading todometer tdistfromlast tspeed taccel tpenwidth tpencolor tagentsize) 
      set result lput datarep result 
    ]
  ]
  report result
