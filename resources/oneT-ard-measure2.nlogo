...
to create-blocks-list
  ...
  [
    ...
    [
      set arg-type "enum"
      set enum-list ["pen-width" "color" "heading"]
    ]
    set arg-list lput max-one-of args [who] arg-list
    hatch-args 1
    [
...
create-blocks 1
  [
    set block-name "forward-sensor-times"
    set category "Sensors"
    set arg-list []
    hatch-args 1
    [
...
set blocks-list lput max-one-of blocks [who] blocks-list
  ...
  create-blocks 1
  [
    set block-name "set-secret-number"
...
to create-agent-kind-list
  ...
  [
    ...
    set primitives-list lput "forward-sensor-times" primitives-list
    ...
    set primitives-list lput "set-secret-number" primitives-list
...
to java-x-plus-sensor
  ...
  set odometer odometer + arduino-distance
  if any? measurepoints
  [
    if distfromlast = NaN
    [set distfromlast 0]
    set distfromlast distfromlast + arduino-distance
  ]
end

to java-x-minus-sensor
  setxy xcor - arduino-distance ycor
  set odometer odometer + arduino-distance
  if any? measurepoints
  [
...
to java-y-plus-sensor
  setxy xcor ycor + arduino-distance
  set odometer odometer + arduino-distance
  if any? measurepoints
  [
    if distfromlast = NaN
...
to java-set-secret-number [a-value]
  set secret-number a-value
end

to java-set-by-sensor [a-value a-operator a-amount]
  let my-dist arduino-distance
...
to java-set-color-by [amount-number]
  if amount-number = "sensor" 
  [set color arduino-distance]
  if amount-number = "four-times-sensor" 
  [set color arduino-distance * 4]
  if amount-number = "ten-times-sensor"
  [set color arduino-distance * 10]
end
...
to-report get-measures
  ...
  [
    ...
    [ 
      ...
      let datarep (list who tcolor (word "\"" [agent-kind-string] of parent "\"") tcycles theading todometer tdistfromlast tspeed taccel) 
...
to-report get-measures-for-filtered [an-agent-kind a-measurepoint-creator]
  ...
  [
    ...
    [ 
      ...
      let datarep (list who tcolor (word "\"" tagentkind "\"") (length result + 1) theading todometer tdistfromlast tspeed taccel) 
      set result lput datarep result 
    ]
  ]
  report result
