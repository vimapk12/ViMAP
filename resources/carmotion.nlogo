...
to recolor-patch  ;; patch procedure
  ...
  [ 
    let a_color scale-color blue pheromone-scent 0.1 5
    if a_color < 55
    [ set a_color blue ]
    set pcolor a_color
  ]
...
to draw
  ...
  let point min-pxcor
  while [point <= max-pxcor] [
  ask patch point 0 [set pcolor black]
    ask patch point -1 [set pcolor black]
      ask patch point  1 [set pcolor black]
      ask patch point min-pycor [set pcolor black]
    ask patch point (min-pycor + 1) [set pcolor black]
      ask patch point  (min-pycor + 2) [set pcolor black]
  set point point + 1
  ]
  
  
  ask n-of 100 patches with [pcolor != sky + 3] [set pcolor grey]
end

to make-cars
  create-wabbits 1 [set shape "car side" setxy min-pxcor + 7 1 set color red set size 15 set heading 90 set agent-kind-string "Red Car"] 
  create-wabbits 1 [set shape "car side" setxy min-pxcor + 7 min-pycor + 2 set color blue - 1 set size 15 set heading 90 set agent-kind-string "Blue Car"]
    ;create-cars1 1 [set shape "car"]
end

to make-clouds
  create-wabbits 1 [set shape "cloud" setxy 0 max-pycor - 7 set color white set size 15 set heading 90 set agent-kind-string "cloud"] 
  create-wabbits 1 [set shape "cloud" setxy 0 -9 set color white set size 15 set heading 90 set agent-kind-string "cloud"] 
  
  create-wabbits 1 [set shape "House Colonial" setxy min-pxcor + 7 4 set color red set size 5 set heading 90 set agent-kind-string "house"] 
  create-wabbits 1 [set shape "House Colonial" setxy min-pxcor + 7 min-pycor + 4 set color blue - 1 set size 5 set heading 90 set agent-kind-string "house"]
    ;create-cars1 1 [set shape "car"]
end
...
to create-agent-kind-list
  ...
  [
    ...
    set primitives-list lput "set-force" primitives-list
...
to java-right 
  set heading 90
  if agent-kind-string = "Red Car" or agent-kind-string = "Blue Car" [set shape "car side"]
end

to java-left 
  set heading 270
  if agent-kind-string = "Red Car" or agent-kind-string = "Blue Car" [set shape "car side west"]
end
