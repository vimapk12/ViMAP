...
to java-pickup-food-here [aWho]
  set has-food true
  if [agent-kind-string] of turtle aWho = "ant" [set shape "ant-has-food"]
  let food-radius 2
...
to java-bottle-kneck 
  if ticks = 100 and  bottle-kneck? = false [
    ask n-of (count wabbits with [agent-kind-string = "ant"] * (.9 )) wabbits with [agent-kind-string = "ant"] [die] set bottle-kneck? true]
end
...
to place-measure-point
  ...
  [
    ...
    if count wabbits with [agent-kind-string = "ant"] > 0 [set antEnergy (mean [color] of wabbits with  [agent-kind-string = "ant"])]
    if count wabbits with [agent-kind-string = "ant"] > 0 [set spiderPopulation (mean [size] of wabbits with  [agent-kind-string = "ant"])]
